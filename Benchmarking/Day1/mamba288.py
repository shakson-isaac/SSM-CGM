...
#             "mamba_kwargs": {
#                 "d_state": 32,
#                 "d_conv": 4,     # mets 4 si tu veux préserver des pics très abrupts
#                 "expand": 2,
#                 "dt_rank": 32,   # si NON supporté par ta lib, supprime cette ligne
#             },
#         },
#         # Post-static: ton "DummyMambaAttn" léger
#         "post_static": {
#             "mamba_depth": 1,
#             "dropout": 0.15,
#             "mamba_kwargs": {
#                 "d_state": 32,
#                 "d_conv": 4,     # mets 4 si tu veux préserver des pics très abrupts # I GET AN ERROR HERE (4 is max!)
#                 "expand": 2,
#                 "dt_rank": 32,   # si NON supporté par ta lib, supprime cette ligne
#             },
#         },
...
param_24h = {
    ...
    "model": {
        "hidden_size": 128,          # ↓ plus stable
        "dropout": 0.20,
        "encoder": {
            "mamba_depth": 4,
            "dropout": 0.20,
            "mamba_kwargs": {
                "d_state": 128,      # ↑ capacité d’état
                "d_conv": 4,         # max utile
                "expand": 4,         # ↑ capacité interne
                "dt_rank": None      # retire si lib ne supporte pas
            },
        },
        "post_static": {
            "mamba_depth": 1,
    ...
    "optim": {
        "type": "AdamW",
        "lr": 1e-3, #3e-4,                  # ↓ vs 1e-3
        "betas": (0.9, 0.95),
        "weight_decay": 5e-4,
    },
    "scheduler": {
        "type": "ReduceLROnPlateau",
        "patience": 15,               # ↓ réagit plus vite
        "factor": 0.2,
        "min_lr": 1e-5,
        "monitor": "val_loss",
        "mode": "min",
...
def TFT_train(train_df, param: dict):
    ...
    first_epoch_timer = FirstEpochTimer(
        ...
        gcs_prefix="checkpoints_mamba288v2"
    )
    # --- GCS checkpoint uploader callback ---
    gcs_ckpt_prefix = "checkpoints_mamba288v2"
...
if __name__ == "__main__":
    ...
    # Train model
    tft, trainer, val_dataloader, train_dataloader, validation, training = TFT_train(train, param_24h) #Pick parameters HERE!!
    # Save model to GCS
    save_tft_to_gcs(tft, model_name="Mamba_12h_288c")