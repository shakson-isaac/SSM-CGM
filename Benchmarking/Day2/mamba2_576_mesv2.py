...
class Mamba2MESIdentical(Mamba2):
    ...
    def __init__(
        ...
        debug_kernel_shapes: bool = False,
        x_share_mode: str = "mean",   # "mean" | "first" | "se"
        se_reduce_ratio: int = 4,     # 👈 ratio de réduction pour SE (H -> H/r -> H)
        **kwargs
    ):
        ...
        self._debug_once = bool(debug_kernel_shapes)
        ...
        # # --- SE (têtes) : petit MLP H -> H/r -> H (utilisé si x_share_mode == "se") ---
        # H = self.nheads
        # r = max(1, int(se_reduce_ratio))
        # r = max(1, H // r) if se_reduce_ratio > 1 else 1  # sécurité si H < r
        # # NB: on veut un bottleneck de taille H//se_reduce_ratio
        # # si tu préfères directement passer "hidden = H // se_reduce_ratio", remplace la logique ci-dessus.
        # hidden = max(1, H // se_reduce_ratio)
        # self.se_fc1 = nn.Linear(H, hidden, bias=True)
        # self.se_fc2 = nn.Linear(hidden, H, bias=True)
    ...
    def _normalize_D(self, device, dtype):
        ...
        if D_raw.dim() == 1:
            ...
            if D_raw.numel() == P:
                return D_raw.view(1, P).expand(H, -1).contiguous()
    ...
    def forward(self, u: torch.Tensor, seq_idx=None):
        ...
        elif self.x_share_mode == "se":
            # ---- SE sur l'axe des têtes (H) ----
            # 1) squeeze sur P: (B,L,H)
            s = x_heads.mean(dim=-1)
            # 2) MLP H -> H/r -> H
            BL = s.shape[0] * s.shape[1]
            H  = s.shape[2]
            a_logits = self.se_fc2(F.relu(self.se_fc1(s.reshape(BL, H)))).reshape(s.shape)  # (B,L,H)
            a = F.softmax(a_logits, dim=2).unsqueeze(-1)                                   # (B,L,H,1)
            # 3) agrégation pondérée des têtes -> (B,L,1,P)
            x_shared = (a * x_heads).sum(dim=2, keepdim=True).contiguous()

        else:
            raise ValueError("x_share_mode doit être 'mean', 'first' ou 'se'")
...
class ResidualMambaBlock(nn.Module):
    def __init__(
        ...
        x_share_mode: str = "mean",     # 👈 optionnel
        se_reduce_ratio: int = 4,       # 👈 optionnel
    ):
...
class DummyMambaAttn(nn.Module):
    ...
    def __init__(
        self, d_model: int, n_head: int = 2, depth: int = 1, dropout: float = 0.0, return_hidden_attn = False, d_state=64,  d_conv=4,  expand=4,  headdim=128,  ngroups=1, dt_rank=None, x_share_mode: str = "mean", se_reduce_ratio: int = 4,
    ):
        super().__init__()
        self.n_head = n_head
        self.mixer = StackedMamba(d_model=d_model, depth=depth, dropout=dropout, return_hidden_attn=return_hidden_attn, d_state=d_state, d_conv = d_conv, expand = expand, headdim = headdim, ngroups = ngroups, x_share_mode=x_share_mode , se_reduce_ratio=se_reduce_ratio,)
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.lstm_encoder = StackedMamba(
            ...
            x_share_mode=mb.get("x_share_mode", "mean"),
            se_reduce_ratio=mb.get("se_reduce_ratio", 4),
        )
...
def TFT_train(train_df, param):
    ...
    first_epoch_timer = FirstEpochTimer(
        ...
        gcs_prefix="checkpoints_mamba576_MESv4"
    )
    # --- GCS checkpoint uploader callback ---
    gcs_ckpt_prefix = "checkpoints_mamba576_MESv4"
    ...
    trainer = Trainer(
        max_epochs=30,
        gradient_clip_val=1.0, #Prior choice: 1.0
...
if __name__ == "__main__":
    ...
    # Train model
    tft, trainer, val_dataloader, train_dataloader, validation, training = TFT_train(train, param_48) #Pick parameters HERE!!
    # Save model to GCS
    save_tft_to_gcs(tft, model_name="Mamba2_12h_576c_MESv4")