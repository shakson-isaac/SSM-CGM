...
# -------------------------
param_24 = {
    "dataset": {"context_length": 120, "horizon": 12, "batch_size": 32},}
...
def Nhits_train(train_df, param):
    ...
    nhits = NHiTS.from_dataset(
        training,
        loss=loss,
        hidden_size=512,                 # ↑ capacité
        n_blocks=[2, 2, 1],              # 3 stacks
        n_layers=[2, 2, 2],
        pooling_sizes=[16, 8, 1],        # high→low (heuri.)
        downsample_frequencies=[2, 1, 1],
        pooling_mode="max",
        batch_normalization=True,
        dropout=0.1,
        learning_rate=0.001,
        weight_decay=1e-2,
        backcast_loss_ratio=0.0,         # (0–0.5 à tester)
        log_interval=10,
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath="checkpoints/",
        filename="nhits_opt-{epoch:02d}-{val_loss:.2f}",
        save_top_k=3, monitor="val_loss", mode="min", save_last=True, save_on_train_epoch_end=True
    )
    first_epoch_timer = FirstEpochTimer("first_epoch_runtime.txt", _BUCKET_NAME, "checkpoints_nhits_OPT")
    gcs_ckpt_prefix = "checkpoints_nhits_OPT"
...
if __name__ == "__main__":
    ...
    save_model_to_gcs(nhits, model_name="NHiTS_12h_OPT")
