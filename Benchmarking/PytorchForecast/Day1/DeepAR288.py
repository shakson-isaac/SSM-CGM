...
GCS_CKPT_PREFIX = "checkpoints_deeparOPT"

param_deepar_24 = {
    "dataset": {"context_length": 288, "horizon": 12, "batch_size": 32},
    "deepar_init": {"hidden_size": 128, "rnn_layers": 2, "dropout": 0.2},
}
param_deepar_48 = {
...
def train_deepar(
    ...
    ckpt_cb = ModelCheckpoint(
        dirpath="checkpoints/",
        filename="deepar288-{epoch:02d}-{val_loss:.2f}",
        monitor="val_loss",
        mode="min",
        save_top_k=3,
        save_last=True,
...
def main():
    ...
    parser.add_argument("--model-name", default="DeepAR_12h_288c", help="Model name for GCS artifacts")
    parser.add_argument("--preset", choices=["24", "48"], default="24", help="Hyperparam preset")
    ...
    # Choose preset
    preset = param_deepar_24 if args.preset == "24" else param_deepar_48
