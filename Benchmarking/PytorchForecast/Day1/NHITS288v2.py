...
def Nhits_train(train_df, param):
    ...
    nhits = NHiTS.from_dataset(
        training,
        hidden_size=64,                 # try 128 for the accuracy preset
        n_layers=1,               # per-block depth ↓
        n_blocks=[1, 1],          # stacks ↓
        pooling_sizes=[2, 2],     # keep it simple
        static_hidden_size=32,    # static tower ↓
        shared_weights=True,      # keep sharing
        dropout=0.1,
        loss=MAE(),  # Use MAE loss for better point forecasts
        learning_rate=1e-3,
        log_interval=50,
    ...
    first_epoch_timer = FirstEpochTimer("first_epoch_runtime.txt", _BUCKET_NAME, "checkpoints_nhits_288v2")
    gcs_ckpt_prefix = "checkpoints_nhits_288v2"
...
if __name__ == "__main__":
    ...
    save_model_to_gcs(nhits, model_name="NHiTS_12h_288cv2")