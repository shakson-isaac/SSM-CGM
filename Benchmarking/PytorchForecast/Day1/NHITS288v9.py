...
def Nhits_train(train_df, param):
    ...
    nhits = NHiTS.from_dataset(
        training,
        hidden_size=48,           # +50% width without overfitting
        n_layers=1,
        n_blocks=[1],
        pooling_sizes=[2],
        static_hidden_size=12,    # slightly smaller static tower
        shared_weights=True,
        dropout=0.1,
        backcast_loss_ratio=0.1,  # <— add light supervision on backcast
        loss=MAE(),
        learning_rate=7e-5,       # tiny LR nudge for stabler MAE training
        log_interval=50,
        reduce_on_plateau_patience=4,
    )
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath="checkpoints/",
        filename="nhits_288-{epoch:02d}-{val_loss:.2f}",
        save_top_k=3, monitor="val_loss", mode="min", save_last=True, save_on_train_epoch_end=True
    )
    first_epoch_timer = FirstEpochTimer("first_epoch_runtime.txt", _BUCKET_NAME, "checkpoints_nhits_288v9")
    gcs_ckpt_prefix = "checkpoints_nhits_288v9"
    ...
    trainer = Trainer(
        ...
        callbacks=[EarlyStopping(monitor="val_loss", patience=1000, mode="min"),
...
if __name__ == "__main__":
    ...
    save_model_to_gcs(nhits, model_name="NHiTS_12h_288cv9")